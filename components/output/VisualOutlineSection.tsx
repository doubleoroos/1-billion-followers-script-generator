  ...
  outline, onSave, onVideoSave, visualStyle, isVeoKeySelected, onSelectKey, onInvalidKeyError 
    ...
    const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
        ...
        if (dragItem.current !== null && dragOverItem.current !== null && dragItem.current !== dragOverItem.current) {
            ...
            // Remap scene numbers to reflect the new order
            const finalOutline = reorderedOutline.map((scene, index) => ({
                ...scene,
                sceneNumber: index + 1,
            }));
                 ...
                 {filteredScenes.length > 0 ? (
                    filteredScenes.map(({ scene, originalIndex }) => (
                        ...
                        >
                            <SceneCard 
                                scene={scene} 
                                onFieldChange={(field, value) => handleSceneFieldChange(originalIndex, field, value)}
...
const SceneCard: React.FC<SceneCardProps> = ({ 
  scene, onFieldChange, onVideoSave, visualStyle, isVeoKeySelected, onSelectKey, onInvalidKeyError, allScenes, completedSceneIds
    ...
    useEffect(() => {
      setIsDurationValid(validateDuration(scene.duration));
    }, [scene.duration]);
