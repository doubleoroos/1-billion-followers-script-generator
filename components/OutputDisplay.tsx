...
const SaveStatusIndicator: React.FC<{ status: SaveStatus }> = ({ status }) => {
    ...
    if (status === 'dirty') content = <span className="text-amber-400">Unsaved changes...</span>;
...
const SceneCard: React.FC<{ scene: Scene, onFieldChange: (field: keyof Scene, value: string) => void, onVideoSave: (scene: Scene) => void, visualStyle: VisualStyle }> = ({ scene, onFieldChange, onVideoSave, visualStyle }) => {
    ...
    const EditableField: React.FC<{label: string, id: string, value: string, field: keyof Scene, isTextarea?: boolean}> = ({ label, id, value, field, isTextarea }) => {
        ...
        <div>
            ...
            {isTextarea ? (
                <textarea id={id} value={value} onChange={(e) => onFieldChange(field, e.target.value)} className={`${commonClasses} h-24`} />
            ) : (
    ...
    return (
        <div className="space-y-16 md:space-y-24">
            <div className="text-center animate-fade-in">
                ...
                <h1 className="text-4xl md:text-5xl font-extrabold text-white mt-4">A Cinematic Treatment</h1>
