...
export const CopyButton: React.FC<CopyButtonProps> = ({ textToCopy, className }) => {
  ...
  const defaultClasses = "absolute top-3 right-3";
  const appliedClass = className !== undefined ? className : defaultClasses;
  ...
  return (
    <button
      onClick={handleCopy}
      className={`btn-glass flex items-center gap-2 text-xs font-semibold py-1 px-3 rounded-full border transition-all duration-300 ${appliedClass} ${
        copied
